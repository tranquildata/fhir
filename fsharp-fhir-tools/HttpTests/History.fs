...
type CarePlanFixture(fhir:FhirClient) =
    ...
    member val CP1 =
        CarePlan(
            ...
            Status = N CarePlan.CarePlanStatus.Draft
        ) |> create
...
let historyTests getFuncStr = [
    sprintf "create/get/update/history - CarePlan - %s" getFuncStr,
        fun (fhirClient, getFunc, historyFync) () ->
            ...
            cp2.Status <- N CarePlan.CarePlanStatus.Active
            ...
            Expect.notEqual cp2_updated.VersionId cp1_read.VersionId "updated resource has different version"
            ...
            // get with id & vid
            let cp2_read_vid = (getFunc cp1.Id (Some cp2_updated.VersionId) :> Resource) :?> CarePlan
            ...
            Expect.equal cp2_read_vid.VersionId cp2_from_his2.VersionId "versionid correct from history"
            ...
            Expect.equal cp2_read_vid.VersionId cp2_from_his3.VersionId "versionid correct from history"
...
let historyTest =
    ...
    let tests = [
        testParam (fhir, Requests.GetResource.ViaGET, Requests.History.ViaGET) (historyTests "GET" |> List.toSeq )
        testParam (fhir, Requests.GetResource.ViaBatch, Requests.History.ViaBatch) (historyTests "Batch" |> List.toSeq )
        testParam (fhir, Requests.GetResource.ViaTransaction, Requests.History.ViaTransaction) (historyTests "Transaction" |> List.toSeq )
    ]
